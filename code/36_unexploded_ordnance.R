...
# load data
## read unexploded ordnance data (source: https://marinecadastre.gov/downloads/data/mc/MunitionsExplosivesConcern.zip)
...
region_data <- data %>%
  # obtain only unexploded ordnance in the study area
...
region_data_hex <- region_hex[region_data, ] %>%
  # spatially join unexploded ordnance values to Gulf of Mexico hex cells
  sf::st_join(x = .,
              y = region_data,
              join = st_intersects) %>%
