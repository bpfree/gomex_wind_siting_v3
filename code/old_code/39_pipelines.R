...
### input directories
#### BOEM pipelines sites
data_dir <- stringr::str_glue("data/a_raw_data/{pattern}.gdb/{pattern}.gdb")
...
region_data <- data %>%
  # obtain only BOEM pipelines in the study area
  rmapshaper::ms_clip(target = .,
                      clip = study_region) %>%
  # create field called "layer" and fill with "BOEM pipelines" for summary
...
# BOEM pipelines hex grids
region_data_hex <- region_hex[region_data, ] %>%
  # spatially join BOEM pipelines values to Gulf of Mexico hex cells
  sf::st_join(x = .,
              y = region_data,
              join = st_intersects) %>%
