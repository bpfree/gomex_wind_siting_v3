...
### input directories
#### final WEA sites
data_dir <- "data/a_raw_data/BOEM-Renewable-Energy-Geodatabase/BOEM_Renewable_Energy_Geodatabase.gdb"
...
# load data
## final WEA data (source: https://www.boem.gov/renewable-energy/boem-renewable-energy-geodatabase)
...
region_data <- data %>%
  # obtain only final WEA in the study area
  rmapshaper::ms_clip(target = .,
                      clip = study_region) %>%
  # create field called "layer" and fill with "final WEA" for summary
...
region_data_hex <- region_hex[region_data, ] %>%
  # spatially join final WEA values to Gulf of Mexico hex cells
  sf::st_join(x = .,
              y = region_data,
              join = st_intersects) %>%
